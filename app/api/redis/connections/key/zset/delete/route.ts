...
export async function POST(request: NextRequest) {
  try {
    const body: ZSetDeleteRequest = await request.json();
    ...
    console.log('[ZSet删除] 请求信息:', { connectionId, dbIndex, actualKey, member });

    let cachedConnection: any = getCachedConnection(connectionId);

    if (!cachedConnection) {
      console.warn(`[ZSet删除] 缓存中连接不存在，尝试自动重连: ${connectionId}`);
      ...
      console.log(`[ZSet删除] 自动重连成功: ${connectionId}`);
    }

    const redisClient = cachedConnection.redis;
    await redisClient.select(dbIndex);

    const affectedRows = await redisClient.zrem(actualKey, member);

    console.log(`[ZSet删除] 删除完成: key="${actualKey}", member="${member}", 影响行数=${affectedRows}`);
