...
export async function POST(request: NextRequest) {
  try {
    ...
    const { connectionId, index, query, from = 0, size = 20 } = body;
    ...
    const response = await client.search({
      ...
      query: query && Object.keys(query).length > 0 ? query : { match_all: {} },
    });
    ...
    console.error('Elastic 查询错误:', error);
    return NextResponse.json(
      { code: 50001, message: `查询失败: ${error?.message || '未知错误'}`, data: null },
      { status: 500 }
    );
  }
}
