...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!connectionId || !index || !doc) {
      return NextResponse.json({ code: 40001, message: '缺少 connectionId/index/doc', data: null }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        result: bodyResult?.result || 'created',
      },
    });
  } catch (error: any) {
    console.error('创建 Elastic 文档错误:', error);
    return NextResponse.json(
