...
export async function POST(request: NextRequest) {
  try {
    ...
    if (name.startsWith('.')) {
      return NextResponse.json({ code: 40301, message: '系统索引禁止删除', data: null }, { status: 403 });
    }
    ...
    const response = await client.indices.delete({ index: name });
    ...
    console.error('删除 Elastic 索引错误:', error);
    return NextResponse.json(
